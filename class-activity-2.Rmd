...
```{r}
breaks = c(0,60,70,75,80,90,100)
histogram <- hist(student$score, breaks = breaks)
```
...
letters <- cut(student$score, breaks = breaks)
```
...
palette1 <- brewer.pal(6, "OrRd")
histogram <- hist(student$score, breaks = breaks, col = palette1)
```
...
boxplot(student$interest, names = c("sport", "music", "nature", "literature"), col = palette2)
```
